...
class Admin::ImageUploadFilterController < ApplicationController
  def upload
    if request.post?
      file_data = params[:file]
      url = "/images/image_upload_filter/#{Time.now.to_i.to_s}_#{rand(1000000)}.jpg"
      index = params[:index]
      out_path = "public#{url}"
      ...
      if file_data.is_a? Tempfile
        ...
        tf = Tempfile.new("temp_image_upload_filter")
        tf << file_data.read
        tf.flush
        path = tf.path
      end
      ...
      @scripts = " onload=\"parent.image_upload_filter_finish_upload(#{index}, '#{url}');\""
    else
      @scripts = ""
    end
